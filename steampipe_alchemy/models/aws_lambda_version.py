...
class AwsLambdaVersionLocal(db.BaseEphemeralModels, FormatMixins):
    __tablename__ = 'aws_lambda_version_local'
    version = Column('version', Text, nullable=True)
    function_name = Column('function_name', Text, nullable=True)
    arn = Column('arn', Text, primary_key=True, nullable=True)
    master_arn = Column('master_arn', Text, nullable=True)
...
cache = select(AwsLambdaVersion).select_from(AwsLambdaVersion)
create_materialized_view('aws_lambda_version_local', cache, db.metadata_materialized)
