...
class AwsVpcSecurityGroupLocal(db.BaseEphemeralModels, FormatMixins):
    __tablename__ = 'aws_vpc_security_group_local'
    group_name = Column('group_name', Text, nullable=True)
    group_id = Column('group_id', Text, nullable=True)
    arn = Column('arn', Text, primary_key=True, nullable=True)
    description = Column('description', Text, nullable=True)
...
cache = select(AwsVpcSecurityGroup).select_from(AwsVpcSecurityGroup)
create_materialized_view('aws_vpc_security_group_local', cache, db.metadata_materialized)
