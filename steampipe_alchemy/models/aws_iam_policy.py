...
class AwsIamPolicy(Base, FormatMixins):
...
class AwsIamPolicyLocal(db.BaseEphemeralModels, FormatMixins):
    __tablename__ = 'aws_iam_policy_local'
    name = Column('name', Text, nullable=True)
    policy_id = Column('policy_id', Text, nullable=True)
    path = Column('path', Text, nullable=True)
    arn = Column('arn', Text, primary_key=True, nullable=True)
...
cache = select(AwsIamPolicy).select_from(AwsIamPolicy)
create_materialized_view('aws_iam_policy_local', cache, db.metadata_materialized)
