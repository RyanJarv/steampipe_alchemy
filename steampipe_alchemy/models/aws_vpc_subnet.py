...
class AwsVpcSubnetLocal(db.BaseEphemeralModels, FormatMixins):
    __tablename__ = 'aws_vpc_subnet_local'
    subnet_id = Column('subnet_id', Text, nullable=True)
    subnet_arn = Column('subnet_arn', Text, nullable=True)
    vpc_id = Column('vpc_id', Text, nullable=True)
    cidr_block = Column('cidr_block', psql.CIDR, nullable=True)
...
cache = select(AwsVpcSubnet).select_from(AwsVpcSubnet)
create_materialized_view('aws_vpc_subnet_local', cache, db.metadata_materialized)
