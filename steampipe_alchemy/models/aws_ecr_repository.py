...
class AwsEcrRepositoryLocal(db.BaseEphemeralModels, FormatMixins):
    __tablename__ = 'aws_ecr_repository_local'
    repository_name = Column('repository_name', Text, nullable=True)
    registry_id = Column('registry_id', Text, nullable=True)
    arn = Column('arn', Text, primary_key=True, nullable=True)
    repository_uri = Column('repository_uri', Text, nullable=True)
...
cache = select(AwsEcrRepository).select_from(AwsEcrRepository)
create_materialized_view('aws_ecr_repository_local', cache, db.metadata_materialized)
