...
class AwsEfsFileSystemLocal(db.BaseEphemeralModels, FormatMixins):
    __tablename__ = 'aws_efs_file_system_local'
    name = Column('name', Text, primary_key=True, nullable=True)
    file_system_id = Column('file_system_id', Text, nullable=True)
    file_system_arn = Column('file_system_arn', Text, nullable=True)
    owner_id = Column('owner_id', Text, nullable=True)
...
cache = select(AwsEfsFileSystem).select_from(AwsEfsFileSystem)
create_materialized_view('aws_efs_file_system_local', cache, db.metadata_materialized)
