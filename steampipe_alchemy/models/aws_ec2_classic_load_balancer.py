...
class AwsEc2ClassicLoadBalancerLocal(db.BaseEphemeralModels, FormatMixins):
    __tablename__ = 'aws_ec2_classic_load_balancer_local'
    name = Column('name', Text, primary_key=True, nullable=True)
    scheme = Column('scheme', Text, nullable=True)
    created_time = Column('created_time', TIMESTAMP, nullable=True)
    vpc_id = Column('vpc_id', Text, nullable=True)
...
cache = select(AwsEc2ClassicLoadBalancer).select_from(AwsEc2ClassicLoadBalancer)
create_materialized_view('aws_ec2_classic_load_balancer_local', cache, db.metadata_materialized)
