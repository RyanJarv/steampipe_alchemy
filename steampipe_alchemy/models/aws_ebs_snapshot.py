...
class AwsEbsSnapshotLocal(db.BaseEphemeralModels, FormatMixins):
    __tablename__ = 'aws_ebs_snapshot_local'
    snapshot_id = Column('snapshot_id', Text, nullable=True)
    state = Column('state', Text, nullable=True)
    volume_size = Column('volume_size', BigInteger, nullable=True)
    volume_id = Column('volume_id', Text, nullable=True)
...
cache = select(AwsEbsSnapshot).select_from(AwsEbsSnapshot)
create_materialized_view('aws_ebs_snapshot_local', cache, db.metadata_materialized)
