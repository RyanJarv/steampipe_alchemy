...
class AwsKmsKeyLocal(db.BaseEphemeralModels, FormatMixins):
    __tablename__ = 'aws_kms_key_local'
    id = Column('id', Text, nullable=True)
    arn = Column('arn', Text, primary_key=True, nullable=True)
    key_manager = Column('key_manager', Text, nullable=True)
    creation_date = Column('creation_date', TIMESTAMP, nullable=True)
...
cache = select(AwsKmsKey).select_from(AwsKmsKey)
create_materialized_view('aws_kms_key_local', cache, db.metadata_materialized)
