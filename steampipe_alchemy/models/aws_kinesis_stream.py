...
class AwsKinesisStream(Base, FormatMixins):
...
class AwsKinesisStreamLocal(db.BaseEphemeralModels, FormatMixins):
    __tablename__ = 'aws_kinesis_stream_local'
    stream_name = Column('stream_name', Text, nullable=True)
    stream_arn = Column('stream_arn', Text, nullable=True)
    stream_status = Column('stream_status', Text, nullable=True)
    stream_creation_timestamp = Column('stream_creation_timestamp', TIMESTAMP, nullable=True)
...
cache = select(AwsKinesisStream).select_from(AwsKinesisStream)
create_materialized_view('aws_kinesis_stream_local', cache, db.metadata_materialized)
