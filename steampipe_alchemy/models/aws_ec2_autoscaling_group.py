...
class AwsEc2AutoscalingGroupLocal(db.BaseEphemeralModels, FormatMixins):
    __tablename__ = 'aws_ec2_autoscaling_group_local'
    name = Column('name', Text, primary_key=True, nullable=True)
    autoscaling_group_arn = Column('autoscaling_group_arn', Text, nullable=True)
    status = Column('status', Text, nullable=True)
    created_time = Column('created_time', TIMESTAMP, nullable=True)
...
cache = select(AwsEc2AutoscalingGroup).select_from(AwsEc2AutoscalingGroup)
create_materialized_view('aws_ec2_autoscaling_group_local', cache, db.metadata_materialized)
