...
class AwsCloudwatchAlarmLocal(db.BaseEphemeralModels, FormatMixins):
    __tablename__ = 'aws_cloudwatch_alarm_local'
    name = Column('name', Text, primary_key=True, nullable=True)
    alarm_arn = Column('alarm_arn', Text, nullable=True)
    state_value = Column('state_value', Text, nullable=True)
    actions_enabled = Column('actions_enabled', Boolean, nullable=True)
...
cache = select(AwsCloudwatchAlarm).select_from(AwsCloudwatchAlarm)
create_materialized_view('aws_cloudwatch_alarm_local', cache, db.metadata_materialized)
