...
class AwsEc2TransitGateway(Base, FormatMixins):
...
class AwsEc2TransitGatewayLocal(db.BaseEphemeralModels, FormatMixins):
    __tablename__ = 'aws_ec2_transit_gateway_local'
    transit_gateway_id = Column('transit_gateway_id', Text, nullable=True)
    transit_gateway_arn = Column('transit_gateway_arn', Text, nullable=True)
    state = Column('state', Text, nullable=True)
    owner_id = Column('owner_id', Text, nullable=True)
...
cache = select(AwsEc2TransitGateway).select_from(AwsEc2TransitGateway)
create_materialized_view('aws_ec2_transit_gateway_local', cache, db.metadata_materialized)
