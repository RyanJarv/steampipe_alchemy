...
class AwsSnsTopicSubscriptionLocal(db.BaseEphemeralModels, FormatMixins):
    __tablename__ = 'aws_sns_topic_subscription_local'
    subscription_arn = Column('subscription_arn', Text, nullable=True)
    topic_arn = Column('topic_arn', Text, nullable=True)
    owner = Column('owner', Text, nullable=True)
    protocol = Column('protocol', Text, nullable=True)
...
cache = select(AwsSnsTopicSubscription).select_from(AwsSnsTopicSubscription)
create_materialized_view('aws_sns_topic_subscription_local', cache, db.metadata_materialized)
