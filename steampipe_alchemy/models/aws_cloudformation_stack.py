...
class AwsCloudformationStackLocal(db.BaseEphemeralModels, FormatMixins):
    __tablename__ = 'aws_cloudformation_stack_local'
    id = Column('id', Text, primary_key=True, nullable=True)
    name = Column('name', Text, nullable=True)
    status = Column('status', Text, nullable=True)
    creation_time = Column('creation_time', TIMESTAMP, nullable=True)
...
cache = select(AwsCloudformationStack).select_from(AwsCloudformationStack)
create_materialized_view('aws_cloudformation_stack_local', cache, db.metadata_materialized)
