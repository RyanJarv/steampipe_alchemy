...
class AwsSagemakerNotebookInstanceLocal(db.BaseEphemeralModels, FormatMixins):
    __tablename__ = 'aws_sagemaker_notebook_instance_local'
    name = Column('name', Text, nullable=True)
    arn = Column('arn', Text, primary_key=True, nullable=True)
    creation_time = Column('creation_time', TIMESTAMP, nullable=True)
    default_code_repository = Column('default_code_repository', Text, nullable=True)
...
cache = select(AwsSagemakerNotebookInstance).select_from(AwsSagemakerNotebookInstance)
create_materialized_view('aws_sagemaker_notebook_instance_local', cache, db.metadata_materialized)
