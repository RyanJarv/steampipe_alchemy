...
class AwsCloudtrailTrail(Base, FormatMixins):
...
class AwsCloudtrailTrailLocal(db.BaseEphemeralModels, FormatMixins):
    __tablename__ = 'aws_cloudtrail_trail_local'
    name = Column('name', Text, nullable=True)
    arn = Column('arn', Text, primary_key=True, nullable=True)
    home_region = Column('home_region', Text, nullable=True)
    is_multi_region_trail = Column('is_multi_region_trail', Boolean, nullable=True)
...
cache = select(AwsCloudtrailTrail).select_from(AwsCloudtrailTrail)
create_materialized_view('aws_cloudtrail_trail_local', cache, db.metadata_materialized)
