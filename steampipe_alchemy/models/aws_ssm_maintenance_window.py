...
class AwsSsmMaintenanceWindowLocal(db.BaseEphemeralModels, FormatMixins):
    __tablename__ = 'aws_ssm_maintenance_window_local'
    name = Column('name', Text, primary_key=True, nullable=True)
    window_id = Column('window_id', Text, nullable=True)
    enabled = Column('enabled', Boolean, nullable=True)
    allow_unassociated_targets = Column('allow_unassociated_targets', Boolean, nullable=True)
...
cache = select(AwsSsmMaintenanceWindow).select_from(AwsSsmMaintenanceWindow)
create_materialized_view('aws_ssm_maintenance_window_local', cache, db.metadata_materialized)
