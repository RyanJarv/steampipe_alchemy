...
class AwsEc2TargetGroupLocal(db.BaseEphemeralModels, FormatMixins):
    __tablename__ = 'aws_ec2_target_group_local'
    target_group_name = Column('target_group_name', Text, nullable=True)
    target_group_arn = Column('target_group_arn', Text, nullable=True)
    target_type = Column('target_type', Text, nullable=True)
    load_balancer_arns = Column('load_balancer_arns', JSON, nullable=True)
...
cache = select(AwsEc2TargetGroup).select_from(AwsEc2TargetGroup)
create_materialized_view('aws_ec2_target_group_local', cache, db.metadata_materialized)
