...
class AwsEc2NetworkLoadBalancer(Base, FormatMixins):
...
class AwsEc2NetworkLoadBalancerLocal(db.BaseEphemeralModels, FormatMixins):
    __tablename__ = 'aws_ec2_network_load_balancer_local'
    name = Column('name', Text, nullable=True)
    arn = Column('arn', Text, primary_key=True, nullable=True)
    type = Column('type', Text, nullable=True)
    scheme = Column('scheme', Text, nullable=True)
...
cache = select(AwsEc2NetworkLoadBalancer).select_from(AwsEc2NetworkLoadBalancer)
create_materialized_view('aws_ec2_network_load_balancer_local', cache, db.metadata_materialized)
