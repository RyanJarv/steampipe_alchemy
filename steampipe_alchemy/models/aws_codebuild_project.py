...
class AwsCodebuildProjectLocal(db.BaseEphemeralModels, FormatMixins):
    __tablename__ = 'aws_codebuild_project_local'
    name = Column('name', Text, nullable=True)
    arn = Column('arn', Text, primary_key=True, nullable=True)
    description = Column('description', Text, nullable=True)
    concurrent_build_limit = Column('concurrent_build_limit', BigInteger, nullable=True)
...
cache = select(AwsCodebuildProject).select_from(AwsCodebuildProject)
create_materialized_view('aws_codebuild_project_local', cache, db.metadata_materialized)
