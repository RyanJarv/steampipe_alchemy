...
class AwsEcsTaskDefinitionLocal(db.BaseEphemeralModels, FormatMixins):
    __tablename__ = 'aws_ecs_task_definition_local'
    task_definition_arn = Column('task_definition_arn', Text, nullable=True)
    cpu = Column('cpu', BigInteger, nullable=True)
    status = Column('status', Text, nullable=True)
    execution_role_arn = Column('execution_role_arn', Text, nullable=True)
...
cache = select(AwsEcsTaskDefinition).select_from(AwsEcsTaskDefinition)
create_materialized_view('aws_ecs_task_definition_local', cache, db.metadata_materialized)
