...
class AwsVpcEipLocal(db.BaseEphemeralModels, FormatMixins):
    __tablename__ = 'aws_vpc_eip_local'
    allocation_id = Column('allocation_id', Text, nullable=True)
    public_ip = Column('public_ip', psql.INET, nullable=True)
    public_ipv4_pool = Column('public_ipv4_pool', Text, nullable=True)
    domain = Column('domain', Text, nullable=True)
...
cache = select(AwsVpcEip).select_from(AwsVpcEip)
create_materialized_view('aws_vpc_eip_local', cache, db.metadata_materialized)
