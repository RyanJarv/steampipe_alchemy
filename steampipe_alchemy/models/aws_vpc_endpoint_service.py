...
class AwsVpcEndpointServiceLocal(db.BaseEphemeralModels, FormatMixins):
    __tablename__ = 'aws_vpc_endpoint_service_local'
    service_name = Column('service_name', Text, nullable=True)
    service_id = Column('service_id', Text, nullable=True)
    owner = Column('owner', Text, nullable=True)
    acceptance_required = Column('acceptance_required', Boolean, nullable=True)
...
cache = select(AwsVpcEndpointService).select_from(AwsVpcEndpointService)
create_materialized_view('aws_vpc_endpoint_service_local', cache, db.metadata_materialized)
