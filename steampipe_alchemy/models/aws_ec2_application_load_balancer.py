...
class AwsEc2ApplicationLoadBalancer(Base, FormatMixins):
...
class AwsEc2ApplicationLoadBalancerLocal(db.BaseEphemeralModels, FormatMixins):
    __tablename__ = 'aws_ec2_application_load_balancer_local'
    name = Column('name', Text, nullable=True)
    arn = Column('arn', Text, primary_key=True, nullable=True)
    type = Column('type', Text, nullable=True)
    scheme = Column('scheme', Text, nullable=True)
...
cache = select(AwsEc2ApplicationLoadBalancer).select_from(AwsEc2ApplicationLoadBalancer)
create_materialized_view('aws_ec2_application_load_balancer_local', cache, db.metadata_materialized)
