...
class AwsIamAccountPasswordPolicyLocal(db.BaseEphemeralModels, FormatMixins):
    __tablename__ = 'aws_iam_account_password_policy_local'
    allow_users_to_change_password = Column('allow_users_to_change_password', Boolean, nullable=True)
    expire_passwords = Column('expire_passwords', Boolean, nullable=True)
    hard_expiry = Column('hard_expiry', Boolean, nullable=True)
    max_password_age = Column('max_password_age', BigInteger, nullable=True)
...
cache = select(AwsIamAccountPasswordPolicy).select_from(AwsIamAccountPasswordPolicy)
create_materialized_view('aws_iam_account_password_policy_local', cache, db.metadata_materialized)
