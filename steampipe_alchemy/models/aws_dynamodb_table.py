...
class AwsDynamodbTableLocal(db.BaseEphemeralModels, FormatMixins):
    __tablename__ = 'aws_dynamodb_table_local'
    name = Column('name', Text, primary_key=True, nullable=True)
    table_arn = Column('table_arn', Text, nullable=True)
    table_id = Column('table_id', Text, nullable=True)
    creation_date_time = Column('creation_date_time', TIMESTAMP, nullable=True)
...
cache = select(AwsDynamodbTable).select_from(AwsDynamodbTable)
create_materialized_view('aws_dynamodb_table_local', cache, db.metadata_materialized)
