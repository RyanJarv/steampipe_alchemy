...
class AwsElasticsearchDomainLocal(db.BaseEphemeralModels, FormatMixins):
    __tablename__ = 'aws_elasticsearch_domain_local'
    domain_name = Column('domain_name', Text, nullable=True)
    domain_id = Column('domain_id', Text, nullable=True)
    arn = Column('arn', Text, primary_key=True, nullable=True)
    elasticsearch_version = Column('elasticsearch_version', Text, nullable=True)
...
cache = select(AwsElasticsearchDomain).select_from(AwsElasticsearchDomain)
create_materialized_view('aws_elasticsearch_domain_local', cache, db.metadata_materialized)
