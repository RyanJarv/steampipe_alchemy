...
class AwsEmrCluster(Base, FormatMixins):
...
class AwsEmrClusterLocal(db.BaseEphemeralModels, FormatMixins):
    __tablename__ = 'aws_emr_cluster_local'
    name = Column('name', Text, nullable=True)
    id = Column('id', Text, primary_key=True, nullable=True)
    cluster_arn = Column('cluster_arn', Text, nullable=True)
    status = Column('status', JSON, nullable=True)
...
cache = select(AwsEmrCluster).select_from(AwsEmrCluster)
create_materialized_view('aws_emr_cluster_local', cache, db.metadata_materialized)
