...
class AwsInspectorAssessmentTemplateLocal(db.BaseEphemeralModels, FormatMixins):
    __tablename__ = 'aws_inspector_assessment_template_local'
    name = Column('name', Text, nullable=True)
    arn = Column('arn', Text, primary_key=True, nullable=True)
    assessment_run_count = Column('assessment_run_count', BigInteger, nullable=True)
    assessment_target_arn = Column('assessment_target_arn', Text, nullable=True)
...
cache = select(AwsInspectorAssessmentTemplate).select_from(AwsInspectorAssessmentTemplate)
create_materialized_view('aws_inspector_assessment_template_local', cache, db.metadata_materialized)
