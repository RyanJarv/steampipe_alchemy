...
class AwsSsmPatchBaselineLocal(db.BaseEphemeralModels, FormatMixins):
    __tablename__ = 'aws_ssm_patch_baseline_local'
    name = Column('name', Text, primary_key=True, nullable=True)
    baseline_id = Column('baseline_id', Text, nullable=True)
    description = Column('description', Text, nullable=True)
    operating_system = Column('operating_system', Text, nullable=True)
...
cache = select(AwsSsmPatchBaseline).select_from(AwsSsmPatchBaseline)
create_materialized_view('aws_ssm_patch_baseline_local', cache, db.metadata_materialized)
