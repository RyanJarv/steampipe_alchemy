...
class AwsBackupPlanLocal(db.BaseEphemeralModels, FormatMixins):
    __tablename__ = 'aws_backup_plan_local'
    name = Column('name', Text, nullable=True)
    arn = Column('arn', Text, primary_key=True, nullable=True)
    backup_plan_id = Column('backup_plan_id', Text, nullable=True)
    creation_date = Column('creation_date', TIMESTAMP, nullable=True)
...
cache = select(AwsBackupPlan).select_from(AwsBackupPlan)
create_materialized_view('aws_backup_plan_local', cache, db.metadata_materialized)
