...
class AwsVpcRouteTableLocal(db.BaseEphemeralModels, FormatMixins):
    __tablename__ = 'aws_vpc_route_table_local'
    route_table_id = Column('route_table_id', Text, nullable=True)
    vpc_id = Column('vpc_id', Text, nullable=True)
    owner_id = Column('owner_id', Text, nullable=True)
    associations = Column('associations', JSON, nullable=True)
...
cache = select(AwsVpcRouteTable).select_from(AwsVpcRouteTable)
create_materialized_view('aws_vpc_route_table_local', cache, db.metadata_materialized)
