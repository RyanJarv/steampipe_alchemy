...
class AwsApiGatewayStageLocal(db.BaseEphemeralModels, FormatMixins):
    __tablename__ = 'aws_api_gateway_stage_local'
    name = Column('name', Text, primary_key=True, nullable=True)
    rest_api_id = Column('rest_api_id', Text, nullable=True)
    deployment_id = Column('deployment_id', Text, nullable=True)
    created_date = Column('created_date', TIMESTAMP, nullable=True)
...
cache = select(AwsApiGatewayStage).select_from(AwsApiGatewayStage)
create_materialized_view('aws_api_gateway_stage_local', cache, db.metadata_materialized)
