...
class AwsIamRoleLocal(db.BaseEphemeralModels, FormatMixins):
    __tablename__ = 'aws_iam_role_local'
    name = Column('name', Text, nullable=True)
    arn = Column('arn', Text, primary_key=True, nullable=True)
    role_id = Column('role_id', Text, nullable=True)
    create_date = Column('create_date', TIMESTAMP, nullable=True)
...
cache = select(AwsIamRole).select_from(AwsIamRole)
create_materialized_view('aws_iam_role_local', cache, db.metadata_materialized)
