...
class AwsSecurityhubProductLocal(db.BaseEphemeralModels, FormatMixins):
    __tablename__ = 'aws_securityhub_product_local'
    name = Column('name', Text, primary_key=True, nullable=True)
    product_arn = Column('product_arn', Text, nullable=True)
    activation_url = Column('activation_url', Text, nullable=True)
    company_name = Column('company_name', Text, nullable=True)
...
cache = select(AwsSecurityhubProduct).select_from(AwsSecurityhubProduct)
create_materialized_view('aws_securityhub_product_local', cache, db.metadata_materialized)
