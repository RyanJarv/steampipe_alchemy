...
class AwsVpcCustomerGatewayLocal(db.BaseEphemeralModels, FormatMixins):
    __tablename__ = 'aws_vpc_customer_gateway_local'
    customer_gateway_id = Column('customer_gateway_id', Text, nullable=True)
    type = Column('type', Text, nullable=True)
    state = Column('state', Text, nullable=True)
    bgp_asn = Column('bgp_asn', Text, nullable=True)
...
cache = select(AwsVpcCustomerGateway).select_from(AwsVpcCustomerGateway)
create_materialized_view('aws_vpc_customer_gateway_local', cache, db.metadata_materialized)
