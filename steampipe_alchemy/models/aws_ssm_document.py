...
class AwsSsmDocumentLocal(db.BaseEphemeralModels, FormatMixins):
    __tablename__ = 'aws_ssm_document_local'
    name = Column('name', Text, primary_key=True, nullable=True)
    account_ids = Column('account_ids', JSON, nullable=True)
    account_sharing_info_list = Column('account_sharing_info_list', JSON, nullable=True)
    approved_version = Column('approved_version', Text, nullable=True)
...
cache = select(AwsSsmDocument).select_from(AwsSsmDocument)
create_materialized_view('aws_ssm_document_local', cache, db.metadata_materialized)
