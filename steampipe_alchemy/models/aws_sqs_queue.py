...
class AwsSqsQueueLocal(db.BaseEphemeralModels, FormatMixins):
    __tablename__ = 'aws_sqs_queue_local'
    queue_url = Column('queue_url', Text, nullable=True)
    queue_arn = Column('queue_arn', Text, nullable=True)
    fifo_queue = Column('fifo_queue', Boolean, nullable=True)
    delay_seconds = Column('delay_seconds', Text, nullable=True)
...
cache = select(AwsSqsQueue).select_from(AwsSqsQueue)
create_materialized_view('aws_sqs_queue_local', cache, db.metadata_materialized)
