...
class AwsSnsTopicLocal(db.BaseEphemeralModels, FormatMixins):
    __tablename__ = 'aws_sns_topic_local'
    topic_arn = Column('topic_arn', Text, nullable=True)
    display_name = Column('display_name', Text, nullable=True)
    owner = Column('owner', Text, nullable=True)
    subscriptions_confirmed = Column('subscriptions_confirmed', BigInteger, nullable=True)
...
cache = select(AwsSnsTopic).select_from(AwsSnsTopic)
create_materialized_view('aws_sns_topic_local', cache, db.metadata_materialized)
