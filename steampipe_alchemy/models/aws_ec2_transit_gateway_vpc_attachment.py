...
class AwsEc2TransitGatewayVpcAttachmentLocal(db.BaseEphemeralModels, FormatMixins):
    __tablename__ = 'aws_ec2_transit_gateway_vpc_attachment_local'
    transit_gateway_attachment_id = Column('transit_gateway_attachment_id', Text, nullable=True)
    transit_gateway_id = Column('transit_gateway_id', Text, nullable=True)
    transit_gateway_owner_id = Column('transit_gateway_owner_id', Text, nullable=True)
    state = Column('state', Text, nullable=True)
...
cache = select(AwsEc2TransitGatewayVpcAttachment).select_from(AwsEc2TransitGatewayVpcAttachment)
create_materialized_view('aws_ec2_transit_gateway_vpc_attachment_local', cache, db.metadata_materialized)
