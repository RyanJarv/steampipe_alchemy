...
class AwsApiGatewayRestApi(Base, FormatMixins):
...
class AwsApiGatewayRestApiLocal(db.BaseEphemeralModels, FormatMixins):
    __tablename__ = 'aws_api_gateway_rest_api_local'
    name = Column('name', Text, primary_key=True, nullable=True)
    api_id = Column('api_id', Text, nullable=True)
    version = Column('version', Text, nullable=True)
    api_key_source = Column('api_key_source', Text, nullable=True)
...
cache = select(AwsApiGatewayRestApi).select_from(AwsApiGatewayRestApi)
create_materialized_view('aws_api_gateway_rest_api_local', cache, db.metadata_materialized)
