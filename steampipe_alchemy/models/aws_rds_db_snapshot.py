...
class AwsRdsDbSnapshot(Base, FormatMixins):
...
class AwsRdsDbSnapshotLocal(db.BaseEphemeralModels, FormatMixins):
    __tablename__ = 'aws_rds_db_snapshot_local'
    db_snapshot_identifier = Column('db_snapshot_identifier', Text, nullable=True)
    arn = Column('arn', Text, primary_key=True, nullable=True)
    type = Column('type', Text, nullable=True)
    status = Column('status', Text, nullable=True)
...
cache = select(AwsRdsDbSnapshot).select_from(AwsRdsDbSnapshot)
create_materialized_view('aws_rds_db_snapshot_local', cache, db.metadata_materialized)
