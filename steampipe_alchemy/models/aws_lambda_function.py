...
class AwsLambdaFunctionLocal(db.BaseEphemeralModels, FormatMixins):
    __tablename__ = 'aws_lambda_function_local'
    name = Column('name', Text, nullable=True)
    code_sha_256 = Column('code_sha_256', Text, nullable=True)
    code_size = Column('code_size', BigInteger, nullable=True)
    description = Column('description', Text, nullable=True)
...
cache = select(AwsLambdaFunction).select_from(AwsLambdaFunction)
create_materialized_view('aws_lambda_function_local', cache, db.metadata_materialized)
