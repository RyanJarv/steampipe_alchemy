...
class AwsRdsDbClusterSnapshot(Base, FormatMixins):
...
class AwsRdsDbClusterSnapshotLocal(db.BaseEphemeralModels, FormatMixins):
    __tablename__ = 'aws_rds_db_cluster_snapshot_local'
    db_cluster_snapshot_identifier = Column('db_cluster_snapshot_identifier', Text, nullable=True)
    arn = Column('arn', Text, primary_key=True, nullable=True)
    type = Column('type', Text, nullable=True)
    status = Column('status', Text, nullable=True)
...
cache = select(AwsRdsDbClusterSnapshot).select_from(AwsRdsDbClusterSnapshot)
create_materialized_view('aws_rds_db_cluster_snapshot_local', cache, db.metadata_materialized)
