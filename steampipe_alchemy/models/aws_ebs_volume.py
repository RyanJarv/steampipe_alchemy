...
class AwsEbsVolumeLocal(db.BaseEphemeralModels, FormatMixins):
    __tablename__ = 'aws_ebs_volume_local'
    volume_id = Column('volume_id', Text, nullable=True)
    arn = Column('arn', Text, primary_key=True, nullable=True)
    volume_type = Column('volume_type', Text, nullable=True)
    state = Column('state', Text, nullable=True)
...
cache = select(AwsEbsVolume).select_from(AwsEbsVolume)
create_materialized_view('aws_ebs_volume_local', cache, db.metadata_materialized)
