...
class AwsS3BucketLocal(db.BaseEphemeralModels, FormatMixins):
    __tablename__ = 'aws_s3_bucket_local'
    name = Column('name', Text, nullable=True)
    arn = Column('arn', Text, primary_key=True, nullable=True)
    creation_date = Column('creation_date', TIMESTAMP, nullable=True)
    bucket_policy_is_public = Column('bucket_policy_is_public', Boolean, nullable=True)
...
cache = select(AwsS3Bucket).select_from(AwsS3Bucket)
create_materialized_view('aws_s3_bucket_local', cache, db.metadata_materialized)
