...
class AwsApiGatewayv2Stage(Base, FormatMixins):
...
class AwsApiGatewayv2StageLocal(db.BaseEphemeralModels, FormatMixins):
    __tablename__ = 'aws_api_gatewayv2_stage_local'
    stage_name = Column('stage_name', Text, nullable=True)
    api_id = Column('api_id', Text, nullable=True)
    api_gateway_managed = Column('api_gateway_managed', Boolean, nullable=True)
    auto_deploy = Column('auto_deploy', Boolean, nullable=True)
...
cache = select(AwsApiGatewayv2Stage).select_from(AwsApiGatewayv2Stage)
create_materialized_view('aws_api_gatewayv2_stage_local', cache, db.metadata_materialized)
